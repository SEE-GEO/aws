...
# read QRNN    
    ...
    im = np.where(np.logical_and(((y_pre[:, 3]).astype(int) == 270), \
                  ...
                  (np.abs(y_pre[:, 5] - y_pre[:, 1])< 20)))[0]  
    for i in im[0:1]:
        ax.plot(y_pre[i, :], quantiles,  '-ro',  linewidth = 2.5)
        
    im = np.where(np.logical_and(((y_pre[:, 3]).astype(int) == 270),\
                                 (np.abs(y_pre[:, 5] - y_pre[:, 1])< 10)))[0]  
    for i in im[0:1]:
        ax.plot(y_pre[i, :], quantiles,  '-bo',  linewidth = 2.5)
        
    im = np.where(np.logical_and(((y_pre[:, 3]).astype(int) == 270),\
                                 (np.abs(y_pre[:, 5] - y_pre[:, 1])> 20)))[0]
   
    for i in im[0:1]:
        ax.plot(y_pre[i, :], quantiles,  '-ko',  linewidth = 2.5)    
ax.set_ylabel('Quantiles')
ax.set_xlabel('TB[K]')
y = np.arange(-3, 4, 1)
#ax.set_yticks(y)
ax.set_yticklabels((r'$3\sigma$', r'$2\sigma$', r'$1\sigma$', 0,r'$1\sigma$', r'$2\sigma$', r'$3\sigma$'), va='center')
...
fig.savefig('Figures/posterior_distribution_I2V.pdf', bbox_inches = 'tight')                               
