...
Fig.~\ref{fig:predicted_errors} shows the comparison of observed errors (prediction accuarcy) to predicted errors. The predicted dataset is constructed by drawing random samples from the posteriori distribution. We do derive any sample from outside $\pm3\sigma$. The predicted error is deviation of random sample to its median. For QRNN-single, the predicted and observed errors have good agreement for the peak of the distribution, but the predicted errors are spread out asymmetrically. For QRNN-all, the predicted and observed errors have a better agreement and their spread is symmetric. The results show that both QRNN-single and QRNN-all is not able to represent the left wings of the distribution. This could also be a sampling issue, as the high errors we try to predict form a very small fraction of the complete dataset.  
...
The five channels are sensitive to different regions of the troposphere so not all clouds get equal representation in each. The cloud impact from regions where the weighting functions match poorly cannot be represented adequately in QRNN. However,the brightness temperature of same cloud measured at different frequencies can be valuable to train QRNN.  Similarly, the clear-sky measurements from different channels can effectively cancel out the noise. QRNN predictions can be interpreted as a weighted average between the multiple channels. As clear-sky cases occur more frequently, the averaging can cancel out noise. 

Finally, we investigate if uncertainty estimates from QRNN-mwi are representative of prediction errors. Fig.~\ref{fig:uncertainty_cloud_impact} displays the average prediction uncertainty in 97\% confidence interval for different cloud impacts. For all five channels, the predictions with small or relatively low cloud signal have a low uncertainty or in other words have high sharpness. As fraction of cloud impact increases, the predictions become increasingly uncertain. Most uncertain predictions occur for the outermost channels (MWI-14, MWI-15). These channels also have the lowest prediction accuracy.
