...
%As the QRNN predictions are in form of quantiles, each prediction has an uncertainty estimate associated with it. Case specific uncertainty estimates can be quite useful but estimates of uncertainty fitted to a Gaussian model of random errors are potentially easier to implement in NWP models. To inspect if the predictions given by QRNN follow a normal distribution, we evaluated the length of confidence intervals from the median. Figure~\ref{fig:prediction_uncertainty_aws-34} shows the prediction uncertainty in AWS-34 for different quantiles. The red line represents the uncertainty in a hypothetical Gaussian distribution of $270\pm0.62$\,K. The high variability in the uncertainty estimates indicates that QRNN can successfully represent uncertainties for multiple types of cases in the individually, rather than having just a single summary measure of uncertainty about the whole scene.

%If the underlying distribution is Gaussian, the prediction uncertainties should be symmetric around the median, and equally spaced for $1\sigma$, $2\sigma$ and so on. The predictions around the median for the 94\% confidence interval ($\pm 2 \sigma$). The figure also shows that the uncertainties for all 1000 random cases are concentrated along a narrow width, thus suggesting that the case to case variability of the uncertainty is low. A similar uncertainty quantification for other 183\,GHz channels also gives similar results.
...
%On the other hand, in order to examine if task difficulty also decreased the accuracy, we plotted the error distribution binned according to the cloud impact. 
