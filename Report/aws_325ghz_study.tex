...
\begin{figure}[!th]
	...
	\caption{Deviation of channel-34 measurements to noise free clear-sky cases. The various curves represent the measurements before and after correction. The correction according to \cite{rekha2012potential} is also shown. The two curves with label ``after correction'' are for two different values of noise threshold. }
...
A simple regression approach is implemented to estimate the clear-sky values from the measurements. From the simulation dataset, 40000 clear-sky and the corresponding all-sky simulations are selected to develop the correction scheme. Without the availability of actual data, an identical simulation dataset but with noise is used mimic the measurements. The noise for individual channels is calculated according to Equation~\ref{eq:noise}. Before deriving any relationship between the different variables, 50\% of the clear-sky simulations are removed from the dataset. This clear-sky removal is to remove any under-representation of cloudy simulations, as discussed in the Validation section. Further, the simulations from channel-34 with cloud impact greater than 25 K are considered as "too cloudy" and are not included in the correction.
...
The correction is applied only to cloudy measurements. All cases with cloud impact less than 0.2 K or greater than 25 K are termed as ``clear'' or ``too cloudy''  respectively and are not corrected. Further, all measurements with TB differences between channel-34 and channel-42 greater than a ``noise-index'' are not adjusted. This check is introduced to avoid correction of clear-sky measurements which are falsely classified as cloudy. the ``noise-index'' is defined as a factor the standard deviation of noise for 183GHz. 

The error in the predicted measurements is assessed by calculating the deviation to noise-free clear-sky observations. The occurrence frequency of the deviations is shown in Figure~\ref{fig:correction:c34-42}. When all measurements are corrected for cloud impact, the deviations should follow a gaussian distribution with zero mean. However, without any correction, the measurements show a strong cloud impact, which is indicated by the non-zero values to the left of the peak (orange line). Removing measurements with cloud impact greater than 25 K (green line) reduces the cloud impact far left but not in the intermediate region indicating that it effectively removes the ``too cloudy'' measurements. After adjustment, the cloudy impact in the measurements is considerably reduced, though the spread increases. The sensitivity of the algorithm to correctly adjust the cloudy cases is tested through two different values of the noise-index: $1 \sigma$ and $2\sigma$. As evident from the figure, both values gave almost similar results indicating that the algorithm can correctly identify and adjust most of the cloudy measurements.      
