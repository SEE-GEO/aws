...
%
% FORMAT convert_mpm89_lines([do_rttov,i_lines])
%
% IN   do_rttov  Apply changes used in RTTOV. Default is false.
%      i_lines   Inlude lines with these indexes (see table in code).
%                Default is to include all.

% 2019-12-27 Patrick Eriksson


function convert_mpm89_lines(do_rttov,i_lines)
%
if nargin < 1 | isempty(do_rttov), do_rttov = false; end
if nargin < 2,                     i_lines = [];     end


% Line data from continuua.cc: 
L = {
      {22.235080, 0.1090, 2.143, 28.11, 0.69, 4.80, 1.00},     %  1
      {67.813960, 0.0011, 8.735, 28.58, 0.69, 4.93, 0.82},     %  2
      {119.995940, 0.0007, 8.356, 29.48, 0.70, 4.78, 0.79},    %  3
      {183.310074, 2.3000, 0.668, 28.13, 0.64, 5.30, 0.85},    %  4
      {321.225644, 0.0464, 6.181, 23.03, 0.67, 4.69, 0.54},    %  5
      {325.152919, 1.5400, 1.540, 27.83, 0.68, 4.85, 0.74},    %  6
      {336.187000, 0.0010, 9.829, 26.93, 0.69, 4.74, 0.61},    %  7
      {380.197372, 11.9000, 1.048, 28.73, 0.69, 5.38, 0.84},   %  8
      {390.134508, 0.0044, 7.350, 21.52, 0.63, 4.81, 0.55},    %  9
      {437.346667, 0.0637, 5.050, 18.45, 0.60, 4.23, 0.48},    % 10
      {439.150812, 0.9210, 3.596, 21.00, 0.63, 4.29, 0.52},    % 11
      {443.018295, 0.1940, 5.050, 18.60, 0.60, 4.23, 0.50},    % 12
      {448.001075, 10.6000, 1.405, 26.32, 0.66, 4.84, 0.67},   % 13
      {470.888947, 0.3300, 3.599, 21.52, 0.66, 4.57, 0.65},    % 14
      {474.689127, 1.2800, 2.381, 23.55, 0.65, 4.65, 0.64},    % 15
      {488.491133, 0.2530, 2.853, 26.02, 0.69, 5.04, 0.72},    % 16
      {503.568532, 0.0374, 6.733, 16.12, 0.61, 3.98, 0.43},    % 17
      {504.482692, 0.0125, 6.733, 16.12, 0.61, 4.01, 0.45},    % 18
      {556.936002, 510.0000, 0.159, 32.10, 0.69, 4.11, 1.00},  % 19
      {620.700807, 5.0900, 2.200, 24.38, 0.71, 4.68, 0.68},    % 20
      {658.006500, 0.2740, 7.820, 32.10, 0.69, 4.14, 1.00},    % 21
      {752.033227, 250.0000, 0.396, 30.60, 0.68, 4.09, 0.84},  % 22
      {841.073593, 0.0130, 8.180, 15.90, 0.33, 5.76, 0.45},    % 23
      {859.865000, 0.1330, 7.989, 30.60, 0.68, 4.09, 0.84},    % 24
      {899.407000, 0.0550, 7.917, 29.85, 0.68, 4.53, 0.90},    % 25
      {902.555000, 0.0380, 8.432, 28.65, 0.70, 5.10, 0.95},    % 26
      {906.205524, 0.1830, 5.111, 24.08, 0.70, 4.70, 0.53},    % 27
      {916.171582, 8.5600, 1.442, 26.70, 0.70, 4.78, 0.78},    % 28
      {970.315022, 9.1600, 1.920, 25.50, 0.64, 4.94, 0.67},    % 29
      {987.926764, 138.0000, 0.258, 29.85, 0.68, 4.55, 0.90}}; % 30
